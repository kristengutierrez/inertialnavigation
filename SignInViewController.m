...
- (IBAction)didTapSignIn:(id)sender {
  //INFO: Sign In with credentials.
  NSString *email = _emailField.text;
  NSString *password = _passwordField.text;
  [[FIRAuth auth] signInWithEmail:email password:password completion:^(FIRUser * _Nullable user, NSError * _Nullable error) {
    if (error) {
      NSLog(@"Could not sign in with email: %@", error.localizedDescription);
      return;
    }
...
- (void)setDisplayName:(FIRUser *)user {
  ...
  //INFO: Use first part of email as the default display name
  changeRequest.displayName = [[user.email componentsSeparatedByString:@"@"] objectAtIndex:0];
  [changeRequest commitChangesWithCompletion:^(NSError *_Nullable error) {
    if (error) {
      NSLog(@"Error in setting display name: %@", error.localizedDescription);
      return;
    }
...
- (IBAction)didRequestPasswordReset:(id)sender {
  ...
  UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    [[FIRAuth auth] sendPasswordResetWithEmail:userInput completion:^(NSError * _Nullable error) {
      if (error) {
        NSLog(@"Unable to reset password: %@", error.localizedDescription);
        return;
      }
    }];
  }];
